...
class WindowMain: ws.wm.Window {
	...
	void drawInput(int[2] pos, int[2] size, int sep){
		...
		// cwd
		int textY = pos.y + ((size.h - draw.to!XDraw.font.h)/2.0).lround.to!int;
		...
		if(!commandBuilder.commandSelected){
			...
			auto xoff = textStart+commandBuilder.commandSelected[0].draw(draw.to!XDraw, [textStart, textY], false, []);
			draw.setColor(options.colorInput);
	...
	void drawOutput(int[2] pos, int[2] size, int sep){
		...
		auto textOffset = ((1.em - draw.to!XDraw.font.h)/2.0).lround.to!int;
		if(selectCurrent < -1){
		...
		foreach(i, match; output[start..min($, start+options.lines+1)]){
			...
			match.draw(draw.to!XDraw, [pos.x+sep+padding, y], start+i == selected, []);
			draw.noclip;
			debug(Score){
				draw.setColor(options.colorInput);
